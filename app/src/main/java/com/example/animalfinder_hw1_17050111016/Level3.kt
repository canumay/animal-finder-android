...
class Level3 : AppCompatActivity(), TextToSpeech.OnInitListener {
    private var timer: Int = 14
    private var bonusPerSecond: Int = 4000 + timer * 200
    private var score: Int = 0
    private var playTimes: Int = 1
    private val animals: MutableList<Animal> = AnimalList().getRandomAnimal(4)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        twLevelTimer.text = "$timer"
        twLevelName.text = "Level 3 (Stage $playTimes/3)"
            ...
            tts.setOnUtteranceProgressListener(object : UtteranceProgressListener() {
                override fun onDone(utteranceId: String) {
                    if(utteranceId == "question") { // if question finished
                        ...
                        if(playTimes == 3) {
                            val intent = Intent(applicationContext, Level4::class.java)
                            intent.putExtra("CURRENT_SCORE", score)
                            finish()
                            startActivity(intent)
                        } else {
                            val intent = Intent(applicationContext, Level3::class.java)
